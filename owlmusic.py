...
# Channels that 0wlmusic Can message.
...
class OwlMusic(discord.Client):
    def __init__(
        ...
        log.info("Initializing 0wlmusic 1.0.0")
        load_opus_lib()
    ...
    async def setup_hook(self) -> None:
        ...
        self.http.user_agent = OWLMUSIC_USER_AGENT_AIOHTTP
        if self.use_certifi:
    ...
    async def _test_network_via_ping(self, ping_target: str) -> int:
        ...
        try:
            ...
            log.error(
                "0wlmusic could not locate a `ping` command path.  Will attempt to use HTTP ping instead."
                "\n0wlmusic tried the following command:   %s"
            ...
            log.error(
                "0wlmusic was denied permission to execute the `ping` command.  Will attempt to use HTTP ping instead."
                "\n0wlmusic tried the following command:   %s"
                "\nYou should enable ping in your system or container environment for best results."
                "\nAlternatively disable network checking via config.",
                " ".join(ping_cmd),
    ...
    def on_network_up(self) -> None:
        """
        Event called by 0wlmusic when it detects network returned from outage.
        """
        log.info("0wlmusic detected network is available again.")
    ...
    def on_network_down(self) -> None:
        """
        Event called by 0wlmusic when it detects network outage.
        """
        log.info("0wlmusic detected a network outage.")
        for gid, player in self.players.items():
        ...
        for guild, channel in channel_map.items():
            ...
            if channel and isinstance(
                ...
                if player:
                    ...
                    log.debug("0wlmusic will make a new MusicPlayer now...")
                    try:
                        player = await self.get_player(
                            channel,
                            create=True,
    ...
    async def get_voice_client(self, channel: VoiceableChannel) -> discord.VoiceClient:
        ...
        :raises: owlmusic.exceptions.PermissionsError
            If 0wlmusic does not have permissions required to join or speak in the `channel`.
        """
        if not isinstance(channel, (discord.VoiceChannel, discord.StageChannel)):
            raise TypeError("Channel passed must be a voice channel")

        # Check if 0wlmusic has required permissions to join in channel.
        chperms = channel.permissions_for(channel.guild.me)
        if not chperms.connect:
            log.error(
                "0wlmusic does not have permission to Connect in channel:  %s",
                channel.name,
            )
            raise exceptions.PermissionsError(
                f"0wlmusic does not have permission to Connect in channel:  `{channel.name}`",
                expire_in=30,
            )
        if not chperms.speak:
            log.error(
                "0wlmusic does not have permission to Speak in channel:  %s",
                channel.name,
            )
            raise exceptions.PermissionsError(
                f"0wlmusic does not have permission to Speak in channel:  `{channel.name}`",
                expire_in=30,
            )
        ...
        for attempt in range(1, (VOICE_CLIENT_MAX_RETRY_CONNECT + 1)):
            ...
            if timeout > max_timeout:
                log.critical(
                    "0wlmusic is unable to connect to the channel right now:  %s",
                    channel,
                )
                raise exceptions.CommandError(
                    "0wlmusic could not connect to the channel. Try again later, or restart the bot if this continues."
                )
            ...
            try:
                ...
                log.voicedebug(  # type: ignore[attr-defined]
                    "0wlmusic has a VoiceClient now..."
                )
                break
                ...
                log.exception(
                    "0wlmusic VoiceClient connection attempt was cancelled. No retry."
                )
                raise exceptions.CommandError(
                    "0wlmusic connection to voice was cancelled. This is odd. Maybe restart?"
                ) from e
    ...
    async def disconnect_voice_client(self, guild: discord.Guild) -> None:
        ...
        for vc in self.voice_clients:
            if not isinstance(vc, discord.VoiceClient):
                log.debug(
                    "0wlmusic has a VoiceProtocol that is not a VoiceClient. Disconnecting anyway..."
                )
                try:
    ...
    async def disconnect_all_voice_clients(self) -> None:
        ...
        for vc in self.voice_clients:
            if isinstance(vc, discord.VoiceClient):
                ...
                log.warning(
                    "0wlmusic.voice_clients list contains a non-VoiceClient object?\n"
    ...
    async def get_player(
        ...
        *,
        ...
        :raises:  owlmusic.exceptions.PermissionsError
            If 0wlmusic is not permitted to join the given `channel`.
        """
        guild = channel.guild
    ...
    def _init_player(
        ...
        """
        Connect a brand-new MusicPlayer instance with the 0wlmusic event
    ...
    async def on_player_finished_playing(self, player: MusicPlayer, **_: Any) -> None:
        ...
        if (
            ...
            while player.autoplaylist:
                ...
                except exceptions.OwlmusicException:
                    log.exception(
                        "0wlmusic needs to stop the autoplaylist extraction and bail."
                    )
                    return
                except Exception:  # pylint: disable=broad-exception-caught
                    log.exception(
                        "0wlmusic got an unhandled exception in player finished event."
                    )
                    break
    ...
    async def on_os_signal(
        ...
        the start of run_owlmusic().
        This allows 0wlmusic to handle external signals and triggering a clean
        shutdown of 0wlmusic in response to them.

        It essentially just calls logout, and the rest of 0wlmusic tear-down is
        finished up in `0wlmusic.run_owlmusic()` instead.
        ...
        try:
            if self and not self.logout_called:
                log.info("Disconnecting and closing down 0wlmusic...")
                await self.logout()
        except Exception as e:
            log.exception("Exception thrown while handling interrupt signal!")
            raise KeyboardInterrupt() from e

    async def run_owlmusic(self) -> None:
        """
        This method is to be used in an event loop to start the 0wlmusic.
        ...
        try:
            ...
            log.info("0wlmusic is now doing shutdown steps...")
            ...
            raise exceptions.HelpfulError(
                ...
                issue="0wlmusic cannot login to discord, is your token correct?",
            ...
            for task in asyncio.all_tasks(loop=self.loop):
                # Don't cancel run_owlmusic task, we need it to finish cleaning.
    ...
    async def logout(self) -> None:
        """
        Disconnect all voice clients and signal 0wlmusic to close it's connections to discord.
        """
    ...
    async def on_resumed(self) -> None:
        ...
        """
        log.info("0wlmusic resumed a session with discord.")
        await self._auto_join_channels(from_resume=True)

    async def on_ready(self) -> None:
        """
        Event called by discord.py typically when 0wlmusic has finished login.
    ...
    async def _on_ready_once(self) -> None:
        ...
        """
        mute_discord_console_log()
        log.debug("Logged in, now getting 0wlmusic ready...")
    ...
    def _gen_embed(self) -> discord.Embed:
        ...
        e.set_footer(
            text=self.config.footer_text, icon_url="https://serverkrisshy.github.io/0wlmusic.jpg"
        )

        # TODO: handle this part when EmbedResponse get handled.
        author_name = "0wlmusic"
        avatar_url = "https://serverkrisshy.github.io/0wlmusic.jpg"
        if self.user:
        ...
        e.set_author(
            name=author_name,
            url="https://github.com/0wlport/0wlmusic",
            icon_url=avatar_url,
        )
        return e
    ...
    def _do_song_blocklist_check(self, song_subject: str) -> None:
        ...
        :raises: owlmusic.exceptions.CommandError
    ...
    async def handle_vc_inactivity(self, guild: discord.Guild) -> None:
        """
        Manage a server-specific event timer when 0wlmusic's voice channel becomes idle,
    ...
    async def cmd_blockuser(
        ...
        if not user_mentions:
            raise exceptions.CommandError(
                "0wlmusic could not find the user(s) you specified.",
                expire_in=20,
            )
    ...
    async def cmd_search(
	    ...
	    example soundcloud: {command_prefix}search sc 7 alan walker alone
	    example youtube   : {command_prefix}search yt 5 maher zain ramadhan
        Searches a service for a video and adds it to the queue.
        - service: any one of the following services:
            - youtube (yt)
            - soundcloud (sc) (default)
        - number: return a number of video results and waits for user to choose one
          - defaults to 10 if unspecified
          - note: If your search query starts with a number,
                  you must put your query in quotes
            - ex: {command_prefix}search 9 "linkin park in the end"
            ...
            if items_requested > max_items:
                raise exceptions.CommandError(
                    self.str.get(
                        ...
                        "You cannot search for more than %s songs",
                    )
                    % max_items
                )
        ...
        search_msg = await self.safe_send_message(
            channel, self.str.get("cmd-search-searching", "Searching songs...")
        )
        await channel.typing()
        ...
        if not info:
            return Response(
                self.str.get("cmd-search-none", "No songs found."), delete_after=30
            )
        ...
        if self.config.searchlist:
            ...
            if self.config.embeds:
                ...
                content.description = "To select a song, tell me the number"
            else:
    ...
    async def cmd_follow(
        ...
        0wlmusic will automatically follow a user when they change channels.
        """
        # If 0wlmusic is already following a user, either change user or un-follow.
        ...
        if author.id == self.config.owner_id and user_mentions:
            ...
            if not isinstance(m, discord.Member):
                raise exceptions.CommandError(
                    "0wlmusic cannot follow a user that is not a member of the server.",
                    expire_in=30,
                )
            bind_to_member = m
    ...
    async def cmd_config(
        ...
        This command allows management of 0wlmusic config options file.
        """
    ...
    async def cmd_queue(
        ...
        if not q_msg:
            ...
            raise exceptions.CommandError(
                "Try that again. 0wlmusic couldn't make or get a reference to the queue message.  If the issue persists, file a bug report."
            )
    ...
    async def cmd_disconnect(self, guild: discord.Guild) -> CommandResponse:
        ...
        for vc in self.voice_clients:
            if not hasattr(vc.channel, "guild"):
                log.warning(
                    "0wlmusic found a %s with no guild!  This could be a problem.",
                    type(vc),
                )
                continue
    ...
    async def cmd_restart(
        ...
        """
        Usage:
            {command_prefix}restart [soft|full|update]
        ...
        `update` updates packages then fully restarts.
        """
        opt = opt.strip().lower()
        if opt not in ["soft", "full", "update"]:
            raise exceptions.CommandError(
                self.str.get(
                    "cmd-restart-invalid-arg",
                    "Invalid option given, use: soft, full, update.",
                ),
                expire_in=30,
            )
        ...
        if opt == "soft":
            ...
            await self.safe_send_message(
                ...
                self.str.get(
                    "cmd-restart-update",
                    "{emoji} Will try to upgrade required packages and restart the bot...",
                ).format(
    ...
    async def cmd_checkupdates(self, channel: MessageableChannel) -> CommandResponse:
        ...
        Display the current bot version and check for updates to 0wlmusic or dependencies.
        """
        pip_status = ""
        updates = False
        ...
        if updates:
            header = "There are updates for 0wlmusic available for download."
        else:
            header = "0wlmusic is totally up-to-date!"
    ...
    async def cmd_uptime(self) -> CommandResponse:
        ...
        Displays the 0wlmusic uptime, since last start/restart.
        """
    ...
    async def cmd_latency(self, guild: discord.Guild) -> CommandResponse:
        ...
        Prints the latency info available to 0wlmusic.
    ...
    async def cmd_botversion(self) -> CommandResponse:
        ...
        """
        return Response(
            "https://github.com/0wlport/0wlmusic\n"
    ...
    async def on_inactivity_timeout_expired(
        ...
        """
        A generic event called by 0wlmusic when configured channel or player
    ...
    async def _handle_api_disconnect(self, before: discord.VoiceState) -> bool:
        """
        Method called from on_voice_state_update when 0wlmusic is disconnected from voice.
        """
    ...
    async def on_guild_channel_update(
        ...
        # The chat delay is not currently respected by 0wlmusic. Is this a problem?
